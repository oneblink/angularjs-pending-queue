...
describe('$http Decorator', () => {
  ...
  describe('when there are no items in the pending queue', () => {
    ...
    xit('should not broadcast either event', (done) => {
      listeners = [
        $rootScope.$on('bmPendingQueueSuccess', () => fail('bmPendingQueueSuccess should not of been broadcast')),
        $rootScope.$on('bmPendingQueueFail', () => fail('bmPendingQueueSuccess should not of been broadcast'))
      ]
  ...
  describe('when there are items in the pending queue', () => {
    ...
    beforeEach((done) => {
      uuids = []
      const save = (memo, val) => {
        return memo.then(() => bmPendingQueueService.save({data: {id: val}, url: '/test', method: 'POST'})
